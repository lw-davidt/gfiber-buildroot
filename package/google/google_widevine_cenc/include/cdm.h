...
class CDM_EXPORT Cdm : public ITimerClient {
 public:
  ...
  // Loads an existing persisted license file from storage. A new session_id is
  // generated.
  ...
  // Exposes the query api of the cdm engine via the ce api.
  virtual Status QueryCryptoID(const std::string&, uint32_t* crypto_id) = 0;
  ...
  struct InputBuffer {
   public:
    ...
    // Used in decryption calls to support side loaded group licenses.
    // TODO(jfore): This is required since the group licenses are sideloaded. As
    // a result the cdm engine knows nothing of the key ids for the loaded
    // group session. This can be eliminated once the cdm engine knows about
    // the loaded key ids.
